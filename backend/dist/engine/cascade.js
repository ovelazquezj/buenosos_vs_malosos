...
// ============================================================
function computeCascadeImpact(service, services, tempEffects) {
    ...
    for (const depId of service.dependencies) {
        ...
        const edgeIgnored = tempEffects.some((e) => e.type === 'ignoreCascadeEdge' &&
...
// ============================================================
function resolveCascades(services, tempEffects, _turn) {
    let current = { ...services };
    let waveChanges = true;
    let waveCount = 0;
    let pendingCriticalChange = null;
    while (waveChanges && waveCount < MAX_WAVES) {
        waveChanges = false;
        waveCount++;
        const next = { ...current };
        const changesThisWave = [];
            ...
            if (waveCount === MAX_WAVES) {
                ...
                for (const change of changesThisWave) {
                    if (!mostCritical) {
                        ...
                        if (change.service.crit > cur.crit ||
...
// ============================================================
function resolveIntermittence(services, turn, tempEffects) {
    ...
    for (const id of Object.keys(services)) {
        ...
        if (svc.state !== 'INTERMITTENT')
            continue;
        // Check if propagation is blocked for this service
        const blocked = tempEffects.some((e) => e.type === 'blockIntermittentPropagation' && e.targetId === id);
        if (blocked)
            continue;
        // Find all dependents (services that list this id in their dependencies)
        const dependents = Object.values(services).filter((s) => s.id !== id && s.dependencies.includes(id) && s.state !== 'DOWN');
        ...
        const newState = newInt === 0 ? 'DOWN' : target.state;
