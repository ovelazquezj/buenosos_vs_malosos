import { GameState, Seat } from '../types/game.types';
...
export function checkVictory(state: GameState): Seat | null {
  ...
  const crit5Down = Object.values(state.services).filter(
    (s) => s.crit === 5 && s.state === 'DOWN'
  ).length;
