import { GameState, Service, TemporaryEffect } from '../types/game.types';

// ============================================================
// CONSTANTS
// ============================================================

const STABILITY_BASE_PENALTY: Record<string, number> = {
  DEGRADED: -2,
  INTERMITTENT: -3,
  DOWN: -6,
};
...
const CRITICALITY_BONUS: Record<number, number> = {
  4: -2,
  5: -4,
};
...
export function calculateTurnMarkers(state: GameState): MarkerUpdate {
  ...
  if (newTrust === 0) {
    ...
    const trustIgnored = state.temporaryEffects.some(
      (e) => e.type === 'ignoreTrustPenalty'
    );
    if (!trustIgnored) {
  ...
  if (prioritization.length > 0) {
    for (const eff of prioritization) {
      const targets = (eff.targets as string[] | undefined) ?? [];
      for (const targetId of targets) {
        const svc = state.services[targetId];
        if (!svc || svc.state === 'OK') continue;
...
export function applyMarkerUpdate(state: GameState, update: MarkerUpdate): GameState {
...
export function modifyTrust(state: GameState, amount: number): GameState {
...
export function modifyStability(state: GameState, amount: number): GameState {
...
export function getEffectiveDamageReduction(
  serviceId: string,
  ...
  for (const eff of tempEffects) {
    if (eff.type === 'damageReductionService' && eff.targetId === serviceId) {
      reduction += (eff.value as number) ?? 0;
    }
    if (eff.type === 'socMonitoring' && eff.targetId === serviceId) {
      reduction += (eff.damageReduction as number) ?? 0;
    }
